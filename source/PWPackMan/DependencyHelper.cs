...
namespace Zbx1425.PWPackMan {
	...
	public static class DependencyHelper {
		...
		public static Tuple<VersionRange, List<Tuple<VersionRange, string[]>>> GetSuitableRange(Context ctx, RemotePackageInfo remotePack) {
			var rangeMap = new Dictionary<VersionRange, HashSet<string>>(); // Value is for blaming packages
			...
			return new Tuple<VersionRange, List<Tuple<VersionRange, string[]>>>(allowedRange, rangesToBlame);
		}
		
		public static void CheckCanRemove(Context ctx, Identifier id) {
		...
		private static Tuple<VersionRange, bool, bool> NarrowerRange(VersionRange a, VersionRange b) {
			...
			return new Tuple<VersionRange, bool, bool>(result, narrowed, result.NotEmpty);
		}
