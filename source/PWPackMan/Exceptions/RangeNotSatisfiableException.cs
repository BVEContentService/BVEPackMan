...
namespace Zbx1425.PWPackMan.Exceptions {
	public class RangeNotSatisfiableException : Exception, ISerializable {
		...
		public RangeNotSatisfiableException(string message, Exception innerException) : base(message, innerException) { }

		public RangeNotSatisfiableException(Context ctx, VersionRange finalRange, List<Tuple<VersionRange, string[]>> blame) : base(GenerateMessage(ctx, finalRange, blame)) { }
		
		// This constructor is needed for serialization.
		protected RangeNotSatisfiableException(SerializationInfo info, StreamingContext context) : base(info, context) { }
		...
		private static string GenerateMessage(Context ctx, VersionRange finalRange, List<Tuple<VersionRange, string[]>> blame) {
			...
			foreach (var item in blame) {
				sb.AppendLine(ctx.Translation.Translate("bpmcore_exception_constraint", item.Item1.ToString(), item.Item2[0], 
